...
namespace Mopsicus.Plugins {
    ...
    public class MobileInput : MonoBehaviour, IPlugin {
        ...
        const string SELF_NAME = "mobileinput";
        ...
        public string Name {
            get
            {
                return SELF_NAME;//GetType ().Name.ToLower ();
            }
        }
            ...
            try {
                ...
                switch (code) {
                    case KEYBOARD_ACTION:
                        ...
                        int leftOffset = (int) (response["lratio"] * (float) Screen.width);
                        int rightOffset = (int) (response["rratio"] * (float) Screen.width);
#elif UNITY_IOS
                        int height = (int) response["height"];
                        int leftOffset = 0;
                        int rightOffset = 0;    
#else
                        int height = (int) response["height"];
                        ...
                        int rightOffset = 0;
#endif
                        OnShowKeyboard?.Invoke(isShow, height, leftOffset, rightOffset);
                        break;
                    default:
                        int id = response["id"];
