...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        public float doStep()
        {
            UnityEngine.Profiling.Profiler.BeginSample("RVO Simulator doStep");
            updateDeleteAgent();
            ...
            for (int block = 0; block < workers_.Length; ++block) workers_[block].step(null);
            // for (int block = 0; block < workers_.Length; ++block) workers_[block].WaitOne();
            for (int block = 0; block < workers_.Length; ++block) workers_[block].update(null);
            // for (int block = 0; block < workers_.Length; ++block) workers_[block].WaitOne();
