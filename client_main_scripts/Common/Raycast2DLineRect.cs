...
public static class Raycast2DLineRect
{
    ...
    public struct LineRectResult
    {
        ...
        public bool HaveHit
        {
            get { return !float.IsPositiveInfinity(entry); }
        }

        public override string ToString()
        {
            return string.Format("t_entry={0}, t_exit={1}", entry, exit);
        }
    }
    ...
    ///         - A line segment from Sector 0 to 5 enters the rect at the  touEdgesching corner 0 (Edge 1 and Edge 3)
    ...
    public static void RaycastLineRect(Vector2 begin, Vector2 end, Rect rect, ref LineRectResult res)
    {
        ...
        Sector hitSide;
        res.entry = GetRayToRectSide(begin, dir, RaycastLookup[sectorBegin, sectorEnd, 0], rect, 0f, out hitSide);
        res.entrySector = hitSide;
        res.exit = GetRayToRectSide(begin, dir, RaycastLookup[sectorBegin, sectorEnd, 1], rect, 1f, out hitSide);
    ...
    private static float GetRayToRectSide(Vector2 begin, Vector2 dir, Sector side, Rect rect, float r4val, out Sector hitSide)
    {
        ...
        switch (side)
        {
            ...
            case Sector.S0:
                curFrac = GetRayToRectSide(begin, dir, Sector.S1, rect, r4val, out hitSide);
                return (float.IsPositiveInfinity(curFrac)) ? GetRayToRectSide(begin, dir, Sector.S3, rect, r4val, out hitSide) : curFrac;

            // Raycast Edge S1 - Horizontal
            case Sector.S1:
                curFrac = RayToHoriz(begin, dir, rect.xMin, rect.yMax, rect.width);
                if (!float.IsPositiveInfinity(curFrac))
                {
                    hitSide = Sector.S1;
                }
                return curFrac;
            ...
            case Sector.S2:
                curFrac = GetRayToRectSide(begin, dir, Sector.S1, rect, r4val, out hitSide);
                return (float.IsPositiveInfinity(curFrac)) ? GetRayToRectSide(begin, dir, Sector.S5, rect, r4val, out hitSide) : curFrac;

            // Raycast Edge S3 - Vertical
            case Sector.S3:
                curFrac = RayToVert(begin, dir, rect.xMin, rect.yMin, rect.height);
                if (!float.IsPositiveInfinity(curFrac))
                {
                    hitSide = Sector.S3;
                }
            ...
            case Sector.S5:
                curFrac = RayToVert(begin, dir, rect.xMax, rect.yMin, rect.height);
                if (!float.IsPositiveInfinity(curFrac))
                {
                    hitSide = Sector.S5;
                }
            ...
            case Sector.S6:
                curFrac = GetRayToRectSide(begin, dir, Sector.S3, rect, r4val, out hitSide);
                return (float.IsPositiveInfinity(curFrac)) ? GetRayToRectSide(begin, dir, Sector.S7, rect, r4val, out hitSide) : curFrac;

            // Raycast Edge S7 - Horizontal
            case Sector.S7:
                curFrac = RayToHoriz(begin, dir, rect.xMin, rect.yMin, rect.width);
                if (!float.IsPositiveInfinity(curFrac))
                {
                    hitSide = Sector.S7;
                }
            ...
            case Sector.S8:
                curFrac = GetRayToRectSide(begin, dir, Sector.S5, rect, r4val, out hitSide);
                return (float.IsPositiveInfinity(curFrac)) ? GetRayToRectSide(begin, dir, Sector.S7, rect, r4val, out hitSide) : curFrac;
