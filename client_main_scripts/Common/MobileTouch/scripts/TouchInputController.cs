...
namespace BitBenderGames
{
	...
	public class TouchInputController
	{
		...
		private float clickDurationThreshold = 1.0f;
		[SerializeField]
		...
		private const float dragFingerChangeThreshold = 400;
		...
		private void InputDonwOnUI()
		{
			...
			else
			{
				if (isFingerDown == false && TouchWrapper.IsFingerDown == true)
				{
					...
					{
                        GameFramework.Log.Debug("--------------------000000");
						isInputOnLockedArea = true;
					}
				}
			}
		...
		public void OnUpdate()
		{
			...
			if (isInputOnLockedArea == false)
			{
				...
				if (isPinching == false)
				{
					...
					{
						if (Vector3.Distance(TouchWrapper.Touch0.Position, lastDragPos) > dragFingerChangeThreshold)
						{
							DragStop(lastFinger0DownPos);
			...
			if (isInputOnLockedArea == false)
			{
				...
				//Log.Info("fingerDownLast Frame True");
			}
		...
		public void SetDragStartDistanceThresholdRelative(float threshold)
		{
			dragStartDistanceThresholdRelative = threshold;
		}
	}
}
