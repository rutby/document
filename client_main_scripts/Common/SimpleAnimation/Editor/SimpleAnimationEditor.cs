...
public class SimpleAnimationEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var animation = target as SimpleAnimation;

        EditorGUILayout.BeginHorizontal();
        testAnimName = EditorGUILayout.TextField("Test Animation:", testAnimName);
        if (GUILayout.Button("Play") && !string.IsNullOrEmpty(testAnimName))
        {
            animation.Play(testAnimName);
        }

        if (GUILayout.Button("Rewind"))
        {
            animation.Rewind(testAnimName);
        }
        
        if (GUILayout.Button("Stop"))
        {
            animation.Stop(testAnimName);
        }
