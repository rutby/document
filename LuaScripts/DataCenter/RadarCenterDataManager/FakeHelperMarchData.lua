...
---
local FakeRescueMarchData = BaseClass("FakeRescueMarchData")
...
local function StartMarch(self)
    if self.curState == FakeMarchState.FAKE_MARCH_STATE_NULL then
        ...
        SFSNetwork.SendMessage(MsgDefines.DetectEventHelpStart, eventUuid)
    ...
    if self.curState == FakeMarchState.FAKE_MARCH_GO_HOME or self.curState == FakeMarchState.FAKE_MARCH_ARRIVE_HOME then
        ...
        if self.uuid ~= nil and (self.callTime == nil or now - self.callTime > retry_gap_time) then
            ...
            if detectData ~= nil and detectData.state == DetectEventState.DETECT_EVENT_STATE_NOT_FINISH then
                ...
                SFSNetwork.SendMessage(MsgDefines.DetectEventHelpEnd, eventUuid)
                
                local playerInfo= nil -- {uid=uid, pic=pic, picVer=picVer}
                ...
                EventManager:GetInstance():Broadcast(EventId.WorldBuildTopBubblePlot, {bUuid = self.uuid, plotId = 6022, playerInfo = playerInfo})
                EventManager:GetInstance():Broadcast(EventId.HelpDetectEndEffectBubbleShow, {bUuid = self.uuid})
            end
        end
    end
end
...
FakeRescueMarchData.__init = __init
FakeRescueMarchData.__delete = __delete
FakeRescueMarchData.SetStartAndEndIndex = SetStartAndEndIndex
FakeRescueMarchData.StartMarch = StartMarch
FakeRescueMarchData.CheckAndStartCollectGarbage = CheckAndStartCollectGarbage
FakeRescueMarchData.CheckAndGoBack = CheckAndGoBack
FakeRescueMarchData.CalculateMarchTime = CalculateMarchTime
FakeRescueMarchData.CalculateCollectGarbageTime = CalculateCollectGarbageTime
FakeRescueMarchData.UpdateState = UpdateState
FakeRescueMarchData.Remove = Remove
FakeRescueMarchData.CheckBackHome = CheckBackHome
FakeRescueMarchData.SendPickEndMessage = SendPickEndMessage
FakeRescueMarchData.NeedRemove = NeedRemove
FakeRescueMarchData.IsEventDoing = IsEventDoing
FakeRescueMarchData.DoWhenBackToWorld = DoWhenBackToWorld

return FakeRescueMarchData