...
local ResLackItem_ItemUseChoose = BaseClass("ResLackItem_ItemUseChoose", ResLackItemBase)
local Localization = CS.GameEntry.Localization
function ResLackItem_ItemUseChoose:CheckIsOk( _resType, _needCnt)
...
function ResLackItem_ItemUseChoose:CheckCount()
...
function ResLackItem_ItemUseChoose:GetConsume()
    local useCount = {}
    local param = {}
...
function ResLackItem_ItemUseChoose:TodoAction(pos,isRefresh,lacktab)

    self:CheckCount()
    local item1 = {}
    item1.itemId = self.itemData.itemId
    local item = DataCenter.ItemData:GetItemById(self._resType)
    local selfNum = 0
    if item ~= nil then
        selfNum = item.count
    end
