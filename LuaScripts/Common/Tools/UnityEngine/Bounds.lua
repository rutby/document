...
-- 做一个临时变量，用来避免临时生成的变量
local minVector3 = Vector3.New()
local maxVector3 = Vector3.New()
...
function Bounds:SetCenter(value)
	self.center = value
end
function Bounds:SetExtents(value)
	self.extents =value
end
function Bounds:GetMin(t)
	t = t or Vector3.New()

	rawset(t, "x", self.center.x - self.extents.x)
	rawset(t, "y", self.center.y - self.extents.y)
	rawset(t, "z", self.center.z - self.extents.z)
	return t
...
function Bounds:SetMin(value)
	self:SetMinMax(value, self:GetMax(maxVector3))
end

function Bounds:GetMax(t)
	t = t or Vector3.New()
...
function Bounds:SetMax(value)
	self:SetMinMax(self:GetMin(minVector3), value)
end

function Bounds:SetMinMax(min, max)
	self.extents.x = (max.x - min.x) * 0.5
	self.extents.y = (max.y - min.y) * 0.5
	self.extents.z = (max.z - min.z) * 0.5

	self.center.x = min.x + self.extents.x
	self.center.y = min.y + self.extents.y
	self.center.z = min.z + self.extents.z
...
function Bounds:Intersects(bounds)
	local min = self:GetMin(minVector3)
	local max = self:GetMax(maxVector3)
...
function Bounds:IntersectRay(ray)
	...
	local t0, t1, f
	local t = self:GetCenter() - ray.origin
	local p = {t.x, t.y, t.z}
	t = self.extents
...
function Bounds:ClosestPoint(point)
	...
	if distance == 0 then
		--return rkPoint, 0
		outPoint = self.center + Vector3.New(closest[1], closest[2], closest[3])
