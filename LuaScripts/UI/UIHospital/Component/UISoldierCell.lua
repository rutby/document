...
local level_text_path = "UISoldierItem/LevelText"
...
local function ReInit(self,param)
	...
	self.level_text:SetText(RomeNum[param.template.show_level])
	self.minValue = 0
	...
	if param.stateType == HospitalPanelStateType.Treating then
		...
		self:SetSliderValue(param.count, true)
	elseif param.stateType == HospitalPanelStateType.Select then
		...
		self:SetSliderValue(self.inputCount / self.maxValue, true)
	end
end
...
local function SetSliderValue(self,value, forceUpdata)
	if self.sliderValue ~= value or forceUpdata == true then
