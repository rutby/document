...
local UIFirstChargeReward = BaseClass("UIFirstChargeReward", UIBaseContainer)
...
local check_ys_path = "UICommonItem/clickBtn/ImgRece/ImgYs"
local img_firstKill = "UICommonItem/clickBtn/Img_FirstKill"

local VFX_Effect_path = "VFX_Effect"
...
local function ComponentDefine(self)
    ...
    --self._effect_vfx = self:AddComponent(UIBaseContainer,VFX_Effect_path)
end
...
-- 全部刷新
local function ReInit(self,param,isReceive)
    self.param = param
    ...
    self:SetVFXState(false)
    if self.param.rewardType and self.param.rewardType == RewardType.HERO then
        if not isReceive  then
            self:SetVFXState(true)
        end
    else
        if self.param.vfx then
            if not isReceive  then
                self:SetVFXState(true)
            end
        end
    ...
    if self.param.rewardType == nil then
        ...
        elseif self.param.rewardType == RewardType.OIL or
               ...
               self.param.rewardType == RewardType.PVE_STAMINA or
...
local function SetVFXState(self,state)
    --self._effect_vfx:SetActive(state)
end

UIFirstChargeReward.OnCreate = OnCreate
UIFirstChargeReward.OnDestroy = OnDestroy
UIFirstChargeReward.OnBtnClick = OnBtnClick
UIFirstChargeReward.OnEnable = OnEnable
UIFirstChargeReward.OnDisable = OnDisable
UIFirstChargeReward.ComponentDefine = ComponentDefine
UIFirstChargeReward.ComponentDestroy = ComponentDestroy
UIFirstChargeReward.DataDefine = DataDefine
UIFirstChargeReward.DataDestroy = DataDestroy
UIFirstChargeReward.ReInit = ReInit
UIFirstChargeReward.SetItemIconImage = SetItemIconImage
UIFirstChargeReward.SetItemQualityImage = SetItemQualityImage
UIFirstChargeReward.SetItemCountActive = SetItemCountActive
UIFirstChargeReward.SetItemCount = SetItemCount
UIFirstChargeReward.SetItemCountColor = SetItemCountColor
UIFirstChargeReward.SetFlagActive = SetFlagActive
UIFirstChargeReward.SetCheckActive = SetCheckActive
UIFirstChargeReward.SetFlagText = SetFlagText
UIFirstChargeReward.SetNameText = SetNameText
UIFirstChargeReward.SetHeroTypeTipPos = SetHeroTypeTipPos
UIFirstChargeReward.GetPosition = GetPosition
UIFirstChargeReward.GetResName = GetResName
UIFirstChargeReward.GetIconPath = GetIconPath
UIFirstChargeReward.SetVFXState = SetVFXState

return UIFirstChargeReward