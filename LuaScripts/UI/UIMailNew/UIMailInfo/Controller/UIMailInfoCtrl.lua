...
local UIMailInfoCtrl = BaseClass("UIMailInfoCtrl", UIBaseCtrl)
...
local function CloseSelf(self)
    UIManager:GetInstance():DestroyWindow(UIWindowNames.UIMailInfo,{anim = false})
end
...
local function OnClickPosBtn(self,pos,serverId,worldId)
    if pos~=0 then
        ...
        GoToUtil.GotoWorldPos(pointV3,nil,nil,nil,serverId,worldId)
    end
...
local function OnClickAlMarkBtn(self, pos, server)
    if pos~=0 then
        GoToUtil.CloseAllWindows()
        GoToUtil.GotoWorldPos(SceneUtils.TileIndexToWorld(pos, ForceChangeScene.World),nil, nil, nil, server)
    end
end
...
UIMailInfoCtrl.OnMoreHero = OnMoreHero
UIMailInfoCtrl.OnUpgradeHeroLevel = OnUpgradeHeroLevel
UIMailInfoCtrl.OnRecruitHero = OnRecruitHero
UIMailInfoCtrl.OnUpgradeHeroQuality = OnUpgradeHeroQuality
UIMailInfoCtrl.CloseSelf = CloseSelf
UIMailInfoCtrl.Close = Close
UIMailInfoCtrl.InitData = InitData
UIMailInfoCtrl.GetCurrentMailListByType = GetCurrentMailListByType
UIMailInfoCtrl.GetOneMailByUid = GetOneMailByUid
UIMailInfoCtrl.GetCurrentMail = GetCurrentMail
UIMailInfoCtrl.DeleteCurrentMail = DeleteCurrentMail
UIMailInfoCtrl.DeleteMailByGroup =DeleteMailByGroup
UIMailInfoCtrl.ReadOneMail = ReadOneMail
UIMailInfoCtrl.ReadMailByGroup =ReadMailByGroup
UIMailInfoCtrl.GetMailWarData = GetMailWarData
UIMailInfoCtrl.GetMailItemOneData =GetMailItemOneData
UIMailInfoCtrl.GetMailCommonData = GetMailCommonData
UIMailInfoCtrl.OnClickPosBtn =OnClickPosBtn
UIMailInfoCtrl.GetCurrentMailUnReadCountTab = GetCurrentMailUnReadCountTab
UIMailInfoCtrl.GetCurrentFightContent = GetCurrentFightContent
UIMailInfoCtrl.GetMailRewardItemList =GetMailRewardItemList
UIMailInfoCtrl.GetMailChatData =GetMailChatData
UIMailInfoCtrl.OnBackMessageClick =OnBackMessageClick
UIMailInfoCtrl.OnNewMailClick = OnNewMailClick
UIMailInfoCtrl.AddMailToFavor = AddMailToFavor
UIMailInfoCtrl.SetCurrentMailId = SetCurrentMailId
UIMailInfoCtrl.OnClickAlMarkBtn = OnClickAlMarkBtn
UIMailInfoCtrl.SetShowTranslated = SetShowTranslated
UIMailInfoCtrl.GetShowTranslated = GetShowTranslated

return UIMailInfoCtrl