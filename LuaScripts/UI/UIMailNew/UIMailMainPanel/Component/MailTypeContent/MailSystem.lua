...
function MailSystem:OnClickBtnGetReward()
    if self._mailData.type == MailType.BE_KING then
...
function MailSystem:setData( maildata )
    ...
    if maildata.type == MailType.COLLECT_OVER_FLOW_MAIL then
        ...
        if type(_strContents) == "string" then
...
function MailSystem:ShowRewardItem(rewardData)
    ...
    if rewardData.rewardType == RewardType.HERO then
        ...
        local rarity = GetTableData(HeroUtils.GetHeroXmlName(), rewardData.itemId, "rarity")
        param.name = string.format("<color=%s>%s</color>",HeroUtils.GetRarityColorStr(rarity),Localization:GetString(heroName))
...
function MailSystem:ShowRewardBtn(maildata)
    ...
    elseif maildata.type == MailType.BE_KING then
        self._objGetReward:SetActive(true)
        self._btnGetReward:SetActive(true)
        self._txtGetReward:SetLocalText(GameDialogDefine.GOTO)
        self._btnGiveBack:SetActive(false)
        ...
        if count~=nil and count>0 and count<= table.count(reward["rewardInfo"]) then
            for i = 1, self._objReward.transform.childCount do
                ...
                if img~=nil then
                    ...
                    local width = 100
                    local height = 100
                    if reward.rewardInfo[i].type == RewardType.ARM then
                        width = 60
                        height = 78
                    end
                    UIUtil.DoFly(reward.rewardInfo[i].type,2,pic,img.gameObject.transform.position,flyPos,width,height)
                end
            end
        end
    end
