local UIALVSDonateSoldierRewardPanelCell = BaseClass("UIALVSDonateSoldierRewardPanelCell", UIBaseContainer)
local base = UIBaseContainer
local UIALVSDonateSoldierRewardPanelItemCell = require "UI.UIALVSDonateSoldier.UIALVSDonateSoldierReward.Component.UIALVSDonateSoldierRewardPanelItemCell"
...
local function OnAddListener(self)
    ...
    self:AddUIListener(EventId.ReceiveALVSDonateArmyStageReward, BindCallback(self, self.OnReceiveDonateSoldierReward))
end

local function OnRemoveListener(self)
    self:RemoveUIListener(EventId.ReceiveALVSDonateArmyStageReward, BindCallback(self, self.OnReceiveDonateSoldierReward))
...
local function SetData(self, info)
    ...
    local selfScore = DataCenter.ActivityALVSDonateSoldierManager:GetScoreNumberByType(0)
    local allianceScore = DataCenter.ActivityALVSDonateSoldierManager:GetScoreNumberByType(1)
...
local function AddRewardItem(self, reward, parent, showMask, showEffect, showCheckmark)
    parent:RemoveComponents(UIALVSDonateSoldierRewardPanelItemCell)
    ...
    for i, v in pairs(reward) do
        if not string.IsNullOrEmpty(v) then
            self:GameObjectInstantiateAsync(UIAssets.DonateSoldierRewardItemCover, function(request)
                ...
                local resComp = parent:AddComponent(UIALVSDonateSoldierRewardPanelItemCell, nameStr)
                local param = {}
...
local function OnRewardBtnClick(self, type)
    ...
    if type == 1 then
        ...
        DataCenter.ActivityALVSDonateSoldierManager:OnReceiveStageReward(type, {self.dataInfo.id})
    elseif type == 2 then 
        ...
        UIManager.Instance:OpenWindow(UIWindowNames.UIALVSDonateSoldierOpenReward, {anim = true}, {{self.dataInfo}, scoreReach})
    end
end

UIALVSDonateSoldierRewardPanelCell.OnCreate = OnCreate
UIALVSDonateSoldierRewardPanelCell.OnDestroy = OnDestroy
UIALVSDonateSoldierRewardPanelCell.OnEnable = OnEnable
UIALVSDonateSoldierRewardPanelCell.OnDisable = OnDisable
UIALVSDonateSoldierRewardPanelCell.OnAddListener = OnAddListener
UIALVSDonateSoldierRewardPanelCell.OnRemoveListener = OnRemoveListener
UIALVSDonateSoldierRewardPanelCell.ComponentDefine = ComponentDefine
UIALVSDonateSoldierRewardPanelCell.ComponentDestroy = ComponentDestroy
UIALVSDonateSoldierRewardPanelCell.SetData = SetData
UIALVSDonateSoldierRewardPanelCell.AddRewardItem = AddRewardItem
UIALVSDonateSoldierRewardPanelCell.OnReceiveDonateSoldierReward = OnReceiveDonateSoldierReward
UIALVSDonateSoldierRewardPanelCell.RefreshClickState = RefreshClickState
UIALVSDonateSoldierRewardPanelCell.OnRewardBtnClick = OnRewardBtnClick

return UIALVSDonateSoldierRewardPanelCell