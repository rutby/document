...
-- UIPiggyBankContent 是两者共用的Component  <= 您在此
...
local function ComponentDefine(self)
    ...
    self.desc_text:SetLocalText(320304)
    self.fill_image = self:AddComponent(UIImage, fill_path)
    self.bubble_desc_text = self:AddComponent(UITextMeshProUGUIEx, bubble_desc_path)
    self.bubble_desc_text:SetLocalText(320303)
    self.seal_desc_text = self:AddComponent(UITextMeshProUGUIEx, seal_desc_path)
    self.seal_num_text = self:AddComponent(UITextMeshProUGUIEx, seal_num_path)
    self.progress_text = self:AddComponent(UITextMeshProUGUIEx, progress_path)
    self.time_desc_text = self:AddComponent(UITextMeshProUGUIEx, time_desc_path)
...
local function OnBuyClick(self)
    ...
    if self.cur >= self.max then
        ...
        local str = Localization:GetString("320306", self.max, self.cur)
...
UIPiggyBankContent.SetOnClose = SetOnClose
UIPiggyBankContent.TimerAction = TimerAction
...
UIPiggyBankContent.OnClose = OnClose
