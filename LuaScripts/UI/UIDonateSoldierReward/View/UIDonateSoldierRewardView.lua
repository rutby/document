local UIDonateSoldierRewardView = BaseClass("UIDonateSoldierRewardView", UIBaseView)
local base = UIBaseView
local UIGiftPackagePoint = require "UI.UIGiftPackage.Component.UIGiftPackagePoint"
local Localization = CS.GameEntry.Localization
local UIDonateSoldierRewardPanelCell = require "UI.UIDonateSoldierReward.Component.UIDonateSoldierRewardPanelCell"
...
local function OnAddListener(self)
    ...
    self:AddUIListener(EventId.GetDonateArmyActivityInfo, self.OnPanelRefresh)
end

local function OnRemoveListener(self)
    self:RemoveUIListener(EventId.GetDonateArmyActivityInfo, self.OnPanelRefresh)
...
local function OnItemMoveIn(self, itemObj, index)
    ...
    local cell = self.scroll_view:AddComponent(UIDonateSoldierRewardPanelCell, itemObj)
    cell:SetData(self.datalist[index])
end

local function OnItemMoveOut(self, itemObj, index)
    self.scroll_view:RemoveComponent(itemObj.name, UIDonateSoldierRewardPanelCell)
end

local function ClearScrollview(self)
    self.scroll_view:ClearCells()
    self.scroll_view:RemoveComponents(UIDonateSoldierRewardPanelCell)
end
...
local function CheckBtnState(self)
    local allNormalRewardStage = DataCenter.ActivityDonateSoldierManager:GetAllCanRecieveNormalReward()
    local idList = {}
    ...
    local allAdvanceRewardStage = DataCenter.ActivityDonateSoldierManager:GetAllCanRecieveAdvanceReward()
    idList = {}
...
local function OnGetAllianceNormalRewardClick(self)
    local allRewardStage = DataCenter.ActivityDonateSoldierManager:GetAllCanRecieveNormalReward()
    local idList = {}
    ...
    if #idList > 0 then
        --有可领取的普通奖励才领
        DataCenter.ActivityDonateSoldierManager:OnReceiveStageReward(1, idList)
    end
end

--领取所有进阶奖励（实际上是打开一个面板）
local function OnGetAllAdvanceRewardClick(self)
    local allRewardStage = DataCenter.ActivityDonateSoldierManager:GetAllCanRecieveAdvanceReward()
    
    if #allRewardStage > 0 then
        UIManager.Instance:OpenWindow(UIWindowNames.UIDonateSoldierOpenReward, {anim = true}, {allRewardStage, true})
    end
end
...
UIDonateSoldierRewardView.OnCreate = OnCreate
UIDonateSoldierRewardView.OnDestroy = OnDestroy
UIDonateSoldierRewardView.OnEnable = OnEnable
UIDonateSoldierRewardView.OnDisable = OnDisable
UIDonateSoldierRewardView.OnRemoveListener = OnRemoveListener
UIDonateSoldierRewardView.OnAddListener = OnAddListener
UIDonateSoldierRewardView.ComponentDefine = ComponentDefine
UIDonateSoldierRewardView.ComponentDestroy = ComponentDestroy
UIDonateSoldierRewardView.SetStaticLocalText = SetStaticLocalText
UIDonateSoldierRewardView.OnItemMoveIn = OnItemMoveIn
UIDonateSoldierRewardView.OnItemMoveOut = OnItemMoveOut
UIDonateSoldierRewardView.ReloadScrollview = ReloadScrollview
UIDonateSoldierRewardView.ClearScrollview = ClearScrollview
UIDonateSoldierRewardView.OnGetAllAdvanceRewardClick = OnGetAllAdvanceRewardClick
UIDonateSoldierRewardView.OnGetAllianceNormalRewardClick = OnGetAllianceNormalRewardClick
UIDonateSoldierRewardView.OnBuyPackageSucc = OnBuyPackageSucc
UIDonateSoldierRewardView.TryShowPackage = TryShowPackage
UIDonateSoldierRewardView.OnPanelRefresh = OnPanelRefresh
UIDonateSoldierRewardView.CheckBtnState = CheckBtnState

return UIDonateSoldierRewardView
