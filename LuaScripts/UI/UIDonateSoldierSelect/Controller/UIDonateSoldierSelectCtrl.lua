local UIDonateSoldierSelectCtrl = BaseClass("UIDonateSoldierSelectCtrl", UIBaseCtrl)
local Localization = CS.GameEntry.Localization

local function CloseSelf(self)
    UIManager:GetInstance():DestroyWindow(UIWindowNames.UIDonateSoldierSelect)
end
...
local function RefreshCurrMaxSoldier(self, initData)
    ...
    table.walksort(freeSoldiers,function (leftKey,rightKey)
        ...
        local score = DataCenter.ActivityDonateSoldierManager:OnGetOneScoreBySoldierId(k)
    ...
    self.maxCanDonateScore = (DataCenter.ActivityDonateSoldierManager:GetTodayDonateSumScore() + currSoldierScore) * DataCenter.ActivityDonateSoldierManager:GetTodayDonateLimitPercent() - DataCenter.ActivityDonateSoldierManager:GetTodayDonateSumScore()
end
...
local function GetCurrLevelNum(self)
    local currLevel = 0
    local currSelfScore = DataCenter.ActivityDonateSoldierManager:GetScoreNumberByType(0)
    local currAllianceScore = DataCenter.ActivityDonateSoldierManager:GetScoreNumberByType(1)

    local stageArr = DataCenter.ActivityDonateSoldierManager:OnGetRewardStageArr()
...
local function GetAddScoreAndLevelNum(self)
    ...
    for _, v in pairs(self.armyList) do
        ...
        local oneScore = DataCenter.ActivityDonateSoldierManager:OnGetOneScoreBySoldierId(soldierId)
...
local function CheckCurrArmyDonateScoreReachMaxLimit(self, armyId, count)
    ...
    --当前兵种
    local curArmyOneScore = DataCenter.ActivityDonateSoldierManager:OnGetOneScoreBySoldierId(armyId)
...
UIDonateSoldierSelectCtrl.CloseSelf = CloseSelf
UIDonateSoldierSelectCtrl.OnSoldierNumberChange = OnSoldierNumberChange
UIDonateSoldierSelectCtrl.OnDonateBtnClick = OnDonateBtnClick
UIDonateSoldierSelectCtrl.GetSoldierList = GetSoldierList
UIDonateSoldierSelectCtrl.GetCurrentSoldierNum = GetCurrentSoldierNum
UIDonateSoldierSelectCtrl.SetCurrentSoldierNum = SetCurrentSoldierNum
UIDonateSoldierSelectCtrl.GetArmyData = GetArmyData
UIDonateSoldierSelectCtrl.RefreshCurrMaxSoldier = RefreshCurrMaxSoldier
UIDonateSoldierSelectCtrl.CheckMax = CheckMax
UIDonateSoldierSelectCtrl.InitSoldierData = InitSoldierData
UIDonateSoldierSelectCtrl.GetAddScoreAndLevelNum = GetAddScoreAndLevelNum
UIDonateSoldierSelectCtrl.GetCurrLevelNum = GetCurrLevelNum
UIDonateSoldierSelectCtrl.OnConfirmDonate = OnConfirmDonate
UIDonateSoldierSelectCtrl.IsDonateSoldierScoreReachMax = IsDonateSoldierScoreReachMax
UIDonateSoldierSelectCtrl.CheckCurrArmyDonateScoreReachMaxLimit = CheckCurrArmyDonateScoreReachMaxLimit

return UIDonateSoldierSelectCtrl