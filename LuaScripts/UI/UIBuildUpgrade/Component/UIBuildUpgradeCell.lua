...
function UIBuildCreateCell:Refresh()
    if self.param.visible then
        ...
        if self.param.param.cellType == CommonCostNeedType.Build then
            local str = Localization:GetString(GetTableData(DataCenter.BuildTemplateManager:GetTableName(), self.param.param.buildId + self.param.param.level,"name")) .. " " .. Localization:GetString(GameDialogDefine.LEVEL_NUMBER, self.param.param.level)
