...
--- Created by Beef.
--- DateTime: 2024/4/7 15:26
---

local base = require "UI.UICityHud.Component.UICityHudBase"
local UICityHudRepairTime = BaseClass("UICityHudRepairTime", base)
...
-- 复用枸杞的逻辑
function UICityHudRepairTime:Update()
    if not self.active then
        return
    end
    local curTime = UITimeManager:GetInstance():GetServerTime()
    local buildData = DataCenter.BuildManager:GetBuildingDataByUuid(self.param.uuid)
    local startTime = buildData.repairStartTime
    local endTime = startTime + BuildRepairTime * 1000
    local changeTime = endTime - curTime
    ...
    if changeTime < maxTime and changeTime > 0 then
        ...
        if maxTime > 0 then
            ...
            if TimeBarUtil.CheckIsNeedChangeBar(changeTime, endTime - self.lastCurTime, maxTime, SliderLength) then
...
UICityHudRepairTime.ComponentDefine = ComponentDefine
UICityHudRepairTime.DataDefine = DataDefine
UICityHudRepairTime.SetParam = SetParam

return UICityHudRepairTime